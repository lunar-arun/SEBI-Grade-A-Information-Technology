Below are **detailed, exam-oriented notes** for **SEBI Grade A – Information Technology (IT) Stream** covering the requested topics from Algorithms and Data Structures.
I’ve structured them to be crisp yet comprehensive, suitable for revision and answer writing.

---

# 📘 **ALGORITHMS FOR PROBLEM SOLVING — DETAILED NOTES**
...
2. For every unvisited node, start DFS/BFS → this forms one connected component
...
## 1. **Kirchhoff’s Matrix–Tree Theorem (general formula)**
...
2. Delete **any one row and the corresponding column** from \(L\) to get a cofactor matrix \(L'\).

3. The **number of spanning trees** is:
\[
\tau(G) = \det(L')
\]

This works for **all graphs**.
...
### (b) Complete bipartite graph \(K_{m,n}\)
\[
...
## 3. **Using eigenvalues (alternative form)**

If \(\lambda_1, \lambda_2, \dots, \lambda_{n-1}\) are the **non-zero eigenvalues** of the Laplacian matrix:
...
| Selection Sort | O(n²)/O(n²)/O(n²)         | O(1)     | No     | Minimum selection           |
...
Absolutely! I’ll create **SEBI Grade A-style MCQs** covering the topics you requested (Algorithms, Trees, Graphs, Hashing, Sorting, Searching, and Design Techniques). I’ll provide **10 questions** followed by **answers at the end**. These will mimic the pattern of SEBI IT stream exams.

---

# **SEBI Grade A – IT Stream: Practice MCQs**
...
**Which edge will Kruskal’s algorithm select first while forming the MST?**
...
1. **B** – In-order traversal of BST gives sorted order.
2. **B** – Any spanning tree has V – 1 edges.
3. **B** – Bellman-Ford works for graphs with negative weights (no negative cycles).
4. **B** – Kruskal picks edge with least weight first → A-B (weight 1).
5. **A** – Linear probing suffers from primary clustering.
6. **B** – Merge sort worst-case: O(n log n).
7. **C** – Fractional Knapsack is solved by greedy.
8. **B** – Binary search worst-case: O(log n).
9. **C** – BFS uses queue; DFS uses stack (or recursion).
10. **C** – Fractional Knapsack uses greedy, not DP.
